...
public class StorageController {
    ...
    public ResponseEntity<byte[]> getMemoryFileByIdImg(@PathVariable("id") String id) {
        FileInfoDTO optionalFile = this.storageService.findById(id);
        if (optionalFile == null) {
            return new ResponseEntity<>(HttpStatus.NOT_FOUND);
        } else {
            HttpHeaders headers = new HttpHeaders();
            headers.setContentType(MediaType.IMAGE_PNG);
            return new ResponseEntity<>(optionalFile.getContent(), headers, HttpStatus.OK);
        }
    }
    ...
    public ResponseEntity<?> uploadFiles(@RequestParam("files") List<MultipartFile> files) {
        ...
        for (MultipartFile file : files) {
            if (isImageFile(file.getOriginalFilename())) {
                try {
                    FileInfoSQL fileInfo = storageService.save(new FileInfoDTO(file.getOriginalFilename(), file.getBytes()));
    ...
    public ResponseEntity<?> uploadFilesJSON(@RequestBody List<JsonFile> files) {
        ...
        for (JsonFile file : files) {
            ...
            if (isImageFile(file.getFileName())) {
                FileInfoSQL fileInfo = storageService.save(new FileInfoDTO(file.getFileName(), file.getContent()));
