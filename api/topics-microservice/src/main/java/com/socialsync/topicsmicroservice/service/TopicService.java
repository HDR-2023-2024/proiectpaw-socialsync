...
*/
    ...
    public void joinTopic(String id, String userId) throws TopicNotFound {
        Optional<Topic> topic = repository.findById(id);

        if (topic.isPresent()) {
            if (topic.get().getMembers().contains(userId)) {
                log.info("User " + userId + "left topic " + id);
                topic.get().getMembers().remove(userId);
                sendMessage(new TopicQueueMessage(QueueMessageType.LEAVE, new Topic(id, userId)));
            }
            else {
                log.info("User " + userId + "joined topic " + id);
                topic.get().getMembers().add(userId);
                sendMessage(new TopicQueueMessage(QueueMessageType.JOIN, new Topic(id, userId)));
            }

            repository.save(topic.get());
        } else
            throw new TopicNotFound("Topic not found.");
    }
}
